...
function Edit () {
    ...
    const checkTask = async () => {
        try {
            const response = await axios.get(`http://localhost:5000/check_task/${id}`);
            ...
            if (error) {
                Swal.fire({
                    ...
                    text:'There have been error in getting task',
                    icon:'error'
                }).then((result) => {
    ...
    const submit = async (e) => {
        ...
        try {
            const response = await axios.put(`http://localhost:5000/update_task/${id}`, {task});

            if (response.data.message === "success")
            {
                Swal.fire({
                    title:'Task Updated',
                    text:'The task has been updated successfuilly',
                    icon:'success'
                }).then((result) => {
                    if (result.isConfirmed) {
                        setTask("");
